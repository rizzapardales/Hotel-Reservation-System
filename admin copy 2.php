...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            $sql = "SELECT 
                        ...
                        cr.reason, rd.total_payment, ad.room_status
                    ...
                    LEFT JOIN admin_status ad ON rd.reserve_id = ad.reserve_id
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>
                                <button type='button' onclick='updateRoomStatus(this)'>Update</button>
                            </td>
                        </tr>";
                }
            } else {
    ...
    <script>
        function updateRoomStatus(button) {
            var row = button.closest('tr');
            var reserveId = row.cells[0].textContent;
            var roomStatus = row.querySelector('select[name="room_status[]"]').value;

            // AJAX request to update room status
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    // Handle response
                    alert(xhr.responseText); // You can show success or error messages here
                }
            };
            xhr.send("reserve_id=" + reserveId + "&room_status=" + roomStatus);
        }
    </script>
</body>
</html>
